    ...
    if numFiles > 0:
       ...
       # cv2.imshow("canny", canny)
        ...
        for index in range(len(squares)):
            ...
            #         end = (i, j)
            #     else:
            #         if a - b <= 10 and b - c <= 10 and a - c <= 10 and (a,b,c)!= (255,255,255):
            #             board[(i,j)] = "R"
            #             start = (i, j)
            #         else:
            #             board[(i,j)] = "X"
        ...
        snapshotAux = "D_D_D_B_B_F"
        #snapshot = ref.get()["sequencia"]
        instructions = computation(start,board,snapshotAux)
        ...
        #playAudioDirectionChanged(board,board)

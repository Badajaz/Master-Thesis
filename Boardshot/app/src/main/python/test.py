...
def verticalLines(file,topLeftX,topLeftY,bottomRightX,bottomRightY):
    image = cv2.imread(file)
    hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)

    fourCorners = getFourCorners((topLeftX, topLeftY),(bottomRightX, bottomRightY))
...
def squares(file,topLeftX,topLeftY,bottomRightX,bottomRightY):
    image = cv2.imread(file)
    hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
